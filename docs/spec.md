# MVP Technical Spec: Pluggable (Disk or S3) PyPI Server

## 1) Overview

* One container with a single API service and an in‑process background worker.
* **Storage backend is pluggable**: `disk` (default) or `s3`.
* The storage (disk directory or S3 bucket) is the source of truth for packages and generated indexes.
...
> **Disk backend:** the layout is rooted at `--data-dir` (default `./pypiron-data`).
> **S3 backend:** the layout lives inside the configured bucket.
...
* **Route:** `POST /` (compatible with redirect-based uploads)
* **Auth:** HTTP Basic (single username/password via env).
* **Flow (both backends):**

  1. Validate `Authorization`; else `401 Unauthorized`.
  2. Require a **filename hint** via `X-Filename` header or `?filename=` query.
  3. Generate an upload URL:

     * **S3:** pre‑signed S3 **PUT** URL.
     * **Disk:** local URL `/_upload/<token>` where the token encodes the final key.
  4. Respond `307 Temporary Redirect` with `Location` set to that URL.
  5. After responding, poll storage for object presence.
  6. Write a job file to `/_internal/queue/pending/`, e.g.  
     `/_internal/queue/pending/<epoch>-<package>-<filename>.json`.

### 3.2) Local Upload Sink (Disk backend only)

* **Route:** `POST|PUT /_upload/:token`
* Body is treated as raw artifact bytes and written to the encoded key.
* Returns `204 No Content` on success.

### 3.3) Download & Serving Endpoints

* **`GET /simple/`**

  * Serve `/simple/index.html` from storage.
* **`GET /simple/<package-name>/`** (and without trailing slash)

  * Serve `/simple/<package-name>/index.html`.
* **`GET /files/<package-name>/<filename>`**

  * Stream `packages/<package-name>/<filename>` from storage to client.
* Notes:

  * Endpoints proxy content (clients never access storage directly).
...
2. **Per‑package indexes** (for each affected package):
   ...
   * Overwrite both files in storage.
...
* Do not expose storage directly; all client traffic goes through the API.
* Paths must tolerate missing trailing slashes and `index.html` omission.
* **Default backend is `disk`**; switch to `s3` via `--storage s3` or `PYPIRON_STORAGE=s3`.
