...
_Audit date: 2026-06-14. Line citations refreshed 2026-06-20 to match the current
tree — the HTTP-only `sync` rewrite shifted most of them; findings are unchanged._
...
#### M2 — Multipart metadata fields are buffered in RAM with no count/aggregate cap (OOM) — **RESOLVED**
- **Update (fixed):** `legacy_upload` now tracks cumulative non-file-part bytes and the distinct-field count, rejecting with 400 past `MAX_METADATA_TOTAL_BYTES` (32 MiB) or `MAX_METADATA_FIELDS` (256) — bounding the metadata map to a few dozen small fields' worth of RAM. Blackbox: `tests/test_upload_limits.py`.
- **Location:** `src/main.rs:1238` (`legacy_upload`); the non-file-part loop at `src/main.rs:1263-1344` (per-field 64 KiB cap at `:1319`).
...
- **Location:** `src/proxy.rs:237` (`ensure_artifact_cached`).
...
- **Location:** `src/origin.rs:62-75`, triggered from `src/proxy.rs:283` (the sole caller — HTTP-only `sync` no longer claims or releases origin markers).
...
#### L4 — Sync buffers whole artifacts and the index JSON in RAM (OOM) — **RESOLVED**
- **Update (fixed):** `sync` now streams each artifact to an `UploadSpool` on disk (hashing as it lands, aborting if the body overruns the upstream-declared `size`) and re-uploads it as a streamed multipart part, so RAM stays chunk-sized regardless of wheel size or concurrency. The index JSON is read through a 64 MiB-capped reader (`simple::read_index_capped`, which also covers the proxy's `fetch_index`). Spool location is `--spool-dir` / `PYPIRON_SYNC_SPOOL_DIR` (default temp; point at real disk, not tmpfs, when mirroring large wheels).
- **Location:** *(was `src/sync.rs` `download_once`, `:1229` `multipart::Part::bytes`, `src/simple.rs:160` index `resp.json()`)*.
...
#### L5 — Crash-heal sidecar records the upstream-claimed size, not the stored bytes — **RESOLVED**
- **Update (HTTP-only sync):** The direct-storage heal path that wrote `write_mirror_sidecar(…, s.file.size.unwrap_or(0))` has been removed. `sync` now POSTs every file through `/legacy/`, and the destination server writes the sidecar from the bytes it actually received (measured and hashed on ingest), not an upstream-claimed `size`. This heal variant no longer exists.
- **Location:** *(was `src/sync.rs:547`, now removed)*.
...
- **Location:** `src/sync.rs:1291` (`matches_filters`); the early `return !has_inclusion_filters` is at `:1327` and `:1334`.
...
- **Location:** `src/worker.rs:533` (classification, `out.dead.push`), `:377` (apply, the `update_global_index(state, &live, &dead)` call), `:898-920` (`update_global_index` removes each dead name — `cached.names.remove(pkg)` at `:920`).
...
- **Location:** `src/worker.rs:737` (`rebuild_package`), `:1179` (`put_if_changed`).
...
#### L9 — Proxy `.metadata`/`.provenance` passthrough loads the upstream response into RAM with no size cap — **RESOLVED**
- **Update (fixed):** both `fetch_metadata_url` and `fetch_provenance_url` now read through `read_capped` (`src/proxy.rs`), which honors Content-Length and `take()`-bounds the streamed body at `wheel::MAX_METADATA_BYTES` (16 MiB), returning `None` (local 404) on overflow — closing the asymmetry with the local extractor.
- **Location:** `src/proxy.rs` `fetch_metadata_url` / `fetch_provenance_url`, via `read_capped`.
- **Risk:** `resp.bytes()` buffers the whole upstream `.metadata` (and `.provenance`) body with no Content-Length/byte ceiling (timeout bounds time, not size). Asymmetric with the local extraction path, which enforces `MAX_METADATA_BYTES` (16 MiB).
...
- **Fix:** Cap both fetches at the same 16 MiB used in `wheel.rs` (Content-Length check and/or a `take()`-limited read); `None` on overflow.
...
- **Location:** `src/proxy.rs:444` (`Proxy::listing`), `:500` (`fetch_listing`).
...
- **Location:** `src/worker.rs:1148` (`backfill_sidecar`), driven by `SIDECAR_READ_CONCURRENCY = 64` (`src/worker.rs:53`, chunked at `:1083`).
...
_Resolved so far: M2, L4, L9 (the upload/sync/proxy buffering OOMs) and L1, L5._

1. **M4** — the remaining upload-path OOM (eager zip central-directory parse); a wheel-size ceiling is the cheap interim.
2. **M3 + L10** — one "bound and coalesce upstream fetches" change for the proxy (the size half, L9, is done).
3. **M1** — the SSRF allowlist/deny-filter; the most code, needs care (DNS rebinding), but the highest-severity open item.
4. **L6** — small, strictly-positive correctness edit (fail closed on unparseable wheels under any filter).
