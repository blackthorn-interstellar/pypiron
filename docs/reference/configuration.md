...
| `--allow-legacy-versions` | `PYPIRON_ALLOW_LEGACY_VERSIONS` | `false` | Mirror files whose version string isn't valid PEP 440. Off by default: an upstream file whose version doesn't parse as PEP 440 is skipped (logged, one line per file) rather than mirrored, so one ancient release can't fail a mirror run. It keys on the version string, not the file type — a `.egg`/`.exe`/`.msi` (or wheel/sdist) with a valid PEP 440 version still mirrors. This is decided entirely on the syncing side: the destination doesn't re-check mirror uploads, so it stores whatever `sync` sends — flip this flag (here, not on the destination) to mirror legacy versions. Also `[sync].allow-legacy-versions`. |
