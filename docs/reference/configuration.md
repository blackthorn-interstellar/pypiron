...
| `--proxy-stream-threshold SIZE` | `PYPIRON_PROXY_STREAM_THRESHOLD` | `16MiB` | Start sending an uncached file this size or larger to the client while it is still downloading from upstream, instead of waiting for the whole thing. The last bytes are withheld until the sha256 check passes, so a download that completes is always a verified one and a bad upstream shows up as a failed transfer. Accepts sizes like `64MB` or `1GiB`, no smaller than `64KiB` (the withheld tail — a smaller threshold refuses to start); `off` waits for every file. |
...
| `--artifact-delivery auto\|redirect\|stream` | `PYPIRON_ARTIFACT_DELIVERY` | `auto` | Redirect object-store downloads when the client handles it well; otherwise stream. A large file nobody has downloaded yet is the exception: above `--proxy-stream-threshold` its very first download comes through the node as it arrives from upstream, because there is no copy in the bucket to redirect to yet. Every download after that redirects as usual. |
...
| `--max-concurrent-artifact-writes N` | `PYPIRON_MAX_CONCURRENT_ARTIFACT_WRITES` | `4` | Cap on how many uploads may buffer their whole body in RAM at once. On an object-store backend a file at or under the 64 MiB single-PUT ceiling is read fully into memory before the conditional PUT, so unbounded concurrent uploads can OOM a small node; the default of `4` bounds that to a ~256 MiB worst case. Only object-store backends are gated — disk hardlinks the spool (~0 RAM) and is never serialized, and a single uncontended upload never waits. `0` means unbounded. |
| `--allow-legacy-versions` | `PYPIRON_ALLOW_LEGACY_VERSIONS` | `false` | Accept uploads whose version string isn't valid PEP 440. Off by default: an upload whose version doesn't parse as PEP 440 is refused with a `400` that names the version and the fix, keeping malformed versions out of the store. It keys on the version string, not the file type — a `.egg`/`.exe`/`.msi` (or any wheel/sdist) with a valid PEP 440 version is still accepted; only a genuinely legacy version is refused, and every modern build tool (pip, uv, twine, build) emits PEP 440. Applies to direct uploads only: mirror uploads bypass this gate, so `sync`'s own [`--allow-legacy-versions`](#sync) is what governs mirrored files. |
...
| `--allow-legacy-versions` | `PYPIRON_ALLOW_LEGACY_VERSIONS` | `false` | Mirror files whose version can't be read from the filename as valid PEP 440. Off by default: sync infers each file's version from its filename, and one it can't parse as PEP 440 is skipped (logged, one line per file) rather than mirrored, so one ancient release can't fail a mirror run. This includes every legacy binary format (`.egg`/`.exe`/`.msi`/`.rpm`/`.dmg`/`.deb`), whose version can't be inferred from the filename at all — those are always skipped by default. This is decided entirely on the syncing side: the destination doesn't re-check mirror uploads, so it stores whatever `sync` sends — flip this flag (here, not on the destination) to mirror those files. Also `[sync].allow-legacy-versions`. |
