...
//!
//! The pure parse/match core lives in [`crate::osv`] (unit-tested and fuzzed
//! there); this module carries the fetch/persist/reload plumbing and the org
//! audit report (exercised blackbox), and re-exports the core types callers
//! reference. The snapshot is a global truth-cache: it is carried verbatim to
//! `_advisories/osv-pypi.zip`, never authored, and a bucket failover must never
//! disarm it.
...
pub use crate::osv::{advisories_for, blocking_advisories, parse_feed, AdvisoryDb};
...
impl AdvisoryState {
    ...
    /// already be [`crate::names::normalize_pkg_name`]-normalized.
    pub fn blocking(&self, name_norm: &str, version: Option<&str>) -> Vec<String> {
        let mut ids: Vec<String> = self
            .db
            .as_ref()
            .map(|db| blocking_advisories(db, name_norm, version))
            .unwrap_or_default()
            .into_iter()
            .map(str::to_string)
            .collect();
...
/// zip per-entry bound.
const MAX_ADVISORY_JSON_BYTES: u64 = crate::osv::MAX_ENTRY_BYTES;
